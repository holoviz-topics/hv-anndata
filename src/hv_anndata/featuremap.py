...
class FeatureMapApp(pn.viewable.Viewer):
    ...
    reduction = param.String(default=None, doc="Initial dimension reduction method", allow_None=True)
    color_by = param.String(default=None, doc="Initial coloring variable", allow_None=True)
    ...
    def __init__(self, **params):
        ...
        if not self.color_by:
            self.color_by = 'cell_type' if 'cell_type' in self.color_options else self.color_options[0]
    ...
    def create_plot(self, dr_key, x_value, y_value, color_value, datashade_value, label_value):

        x_data = self.adata.obsm[dr_key]
        dr_label = self.get_reduction_label(dr_key)
        
        if x_value == y_value:
            return pn.pane.Markdown("Please select different dimensions for X and Y axes.")
        ...
        try:
            ...
            try:
                color_data = self.adata.X.getcol(self.adata.var_names.get_loc(color_value)).toarray().flatten()
    ...
    def __panel__(self):
        ...
        # Widgets
        dr_select = pn.widgets.Select.from_param(self.param.reduction, options=self.dr_options)
        ...
        color = pn.widgets.Select.from_param(self.param.color_by, options=self.color_options)
        datashade_switch = pn.widgets.Checkbox.from_param(self.param.datashade, name="Datashader Rasterize For Large Datasets")
        label_switch = pn.widgets.Checkbox.from_param(self.param.labels, name="Overlay Labels For Categorical Coloring")
